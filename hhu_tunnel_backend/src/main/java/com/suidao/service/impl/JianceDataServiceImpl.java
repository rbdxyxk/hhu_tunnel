...
import com.suidao.pojo.JianceYuzhi;
 ...
 * @description 针对表【jiance_data(检测数据表)】的数据库操作Service实现
 * @createDate 2023-03-15 11:02:07
 */
@Service
@Slf4j
public class JianceDataServiceImpl implements JianceDataService {
    @Resource
    ...
    @Resource
    JianceYuzhiService jianceYuzhiService;
...
//        dynamicTableQueryWrapper.eq(SqlConf.firstIndexId, map.get("firstIndexId"));
    ...
    public Map add(Map map, Long current, Long limit) {
        //根据itemIndexId来获取表头数据
        Long itemIndexId = Long.valueOf(map.get("itemIndexId").toString());
        List<JianceTablehead> tableHead = jianceTableheadService.getTableHeadWithoutMap(itemIndexId);
        HashMap<String, String> headMap = new HashMap<>();
        for (JianceTablehead jianceTablehead : tableHead) {
    ...
    //获取数据
    public Map<String, Object> getData(JianceDynamicTable jianceDynamicTable, Long current, Long limit, Map map) {
        ...
        //获取总数据个数
        Long total = dynamicTableService.getTotal(jianceDynamicTable.getTableName(), suidaoId, firstIndexId, secondIndexId, intemIndxId);
        if (total == 0)
            return null;
        Long begin = (current - 1) * limit;
        //数据list
        List<Map<String, Object>> tableDataList = dynamicTableService.getDataByTableName(jianceDynamicTable.getTableName(), begin, limit, suidaoId, firstIndexId, secondIndexId, intemIndxId);
    ...
    public Result getYuzhi(Map map) {
        ...
        //根据两个去查询阈值表
        QueryWrapper<JianceYuzhi> jianceYuzhiQueryWrapper = new QueryWrapper<>();
        jianceYuzhiQueryWrapper.eq(SqlConf.firstIndexId, firstIndexId);
        jianceYuzhiQueryWrapper.eq(SqlConf.itemId, itemIndexId);
        List<JianceYuzhi> yuzhiList = jianceYuzhiService.list(jianceYuzhiQueryWrapper);
        ...
        for (JianceYuzhi jianceYuzhi : yuzhiList) {
